...
def diagnostic_summary(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if row is None:
        return {"reference_k": None, "failed_guardrails": failed, "leaders": leaders, "state_separation": None, "oos_consistency": None}
    ...
    return {
        ...
        "oos_consistency": {
            "likelihood_drift": aggregate["train_oos_likelihood_drift"]["mean"],
            "occupancy_drift_l1": aggregate["occupancy_drift_l1"]["mean"],
        },
    }
...
def materially_clearer(baseline: dict[str, Any], variant: dict[str, Any]) -> bool:
    ...
    if left is None or right is None:
        return False
    left_agg, right_agg = left["aggregate"], right["aggregate"]
    separation = right_agg["minimum_pairwise_separation"]["mean"] >= (1.0 + MATERIAL_IMPROVEMENT) * left_agg["minimum_pairwise_separation"]["mean"]
    left_ll = left_agg["train_oos_likelihood_drift"]["mean"]
    right_ll = right_agg["train_oos_likelihood_drift"]["mean"]
    left_occ = left_agg["occupancy_drift_l1"]["mean"]
    right_occ = right_agg["occupancy_drift_l1"]["mean"]
    consistency = (
...
def choose_feature_set(variants: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for name in ("baseline_er", "baseline_er_downside"):
        ...
        if variant["decision"]["selected_k"] is not None and materially_clearer(baseline, variant):
            return {"outcome": "select_feature_set", "selected_feature_set": name, "selected_k": variant["decision"]["selected_k"], "reason": "This is the simplest feature set with a stable internal K decision and at least 10% better separation plus non-worsening OOS consistency, including at least one 10% consistency improvement, versus baseline at the same K."}
    return {"outcome": "keep_productization_paused", "selected_feature_set": None, "selected_k": None, "reason": "None of the three feature sets supports both a stable fixed-K decision and materially clearer state separation/OOS consistency."}
...
def compare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return compare_state_counts.strict_json({
        ...
        "cross_feature_policy": {"material_improvement_fraction": MATERIAL_IMPROVEMENT, "likelihood_alone_is_sufficient": False},
        "variants": variants,
...
def markdown_report(result: dict[str, Any]) -> str:
    ...
    lines = ["# SPY 1D HMM feature-sufficiency decision", "", f"**Outcome:** `{decision['outcome']}`", f"**Feature set:** `{decision['selected_feature_set'] or 'none'}`", f"**Selected K:** {decision['selected_k'] if decision['selected_k'] is not None else 'none'}", "", decision["reason"], "", "| Feature set | Internal decision | Leaders AIC/BIC/OOS LL | Failed guardrails | Separation | LL drift / occupancy drift | Reason |", "|---|---|---|---|---:|---:|---|"]
    for name in FEATURE_SETS:
        ...
        separation = f"{summary['state_separation']:.3f}" if summary["state_separation"] is not None else "—"
        consistency = summary["oos_consistency"]
        consistency_text = f"{consistency['likelihood_drift']:.3f} / {consistency['occupancy_drift_l1']:.3f}" if consistency else "—"
        reason = variant["decision"]["reason"]
        lines.append(f"| `{name}` | {internal} | {leader_text} | {failed} | {separation} | {consistency_text} | {reason} |")
    lines += ["", "All three variants use the unchanged #26 split, training-only scaler, K=3–8 HMM configuration, seeds/restarts, causal filter, alignment, diagnostics, guardrails, and internal decision logic. The 10% rule applies only to the new cross-feature-set materiality decision; likelihood improvement alone cannot select a richer feature set.", ""]
    return "\n".join(lines)
