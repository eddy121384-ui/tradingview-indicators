...
def align_to_anchor(series_map: dict[str, pd.Series], anchor: str = "spy") -> pd.DataFrame:
    ...
    for canonical, series in series_map.items():
        ...
        frame[canonical] = clean.reindex(union_index).ffill().reindex(calendar)
...
def build_public_sources(start: str = "2007-01-01", end: str | None = None, *,
    ...
    manifest = {
        ...
        "alignment_semantics": "Forward-fill on the union of source and anchor dates, then select anchor trading dates; never backfill before first observation.",
        "parity_warning": (
