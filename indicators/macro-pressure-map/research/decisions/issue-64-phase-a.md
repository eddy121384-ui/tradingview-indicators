...
The strongest stable relationship is **Reflation / Inflation Rising → SPY outperforming TLT**. This relationship survives a pre-2020 development slice and a post-2019 reused exploratory slice at both the 1M and 3M horizons with direct paired-return confidence intervals above zero in both eras.

By contrast, many other apparent full-history regime winners change when the history is split. The correct Phase-A conclusion is therefore not “the nine cells map cleanly to nine portfolio recipes.” It is “some regime-conditioned relative-asset relationships are real enough to justify a constrained portfolio test, while many cells are era-sensitive and should default to a neutral diversified allocation.”
...
Across 81 direct pairwise regime / horizon comparisons (SPY−TLT, SPY−GLD, TLT−GLD), only **39 / 81** retain the same point-estimate sign across the two eras. Only **2 / 81** have 95% paired-spread intervals excluding zero in the same direction in both eras.

## Strongest stable relationship — Reflation: SPY over TLT

Direct paired-return spread = SPY total return minus TLT total return.

### 1M

- development: **+1.41%**, 95% CI **[+0.01%, +2.74%]**, n=51 embargoed starts;
- post-2019 reused exploratory: **+2.31%**, 95% CI **[+0.70%, +4.01%]**, n=22.

### 3M

- development: **+4.17%**, 95% CI **[+1.41%, +7.02%]**, n=23;
- post-2019 reused exploratory: **+6.42%**, 95% CI **[+2.71%, +10.39%]**, n=11.

### 6M

- development: +4.58%, 95% CI [-4.17%, +12.32%], n=15;
- post-2019 reused exploratory: +12.00%, 95% CI [+6.75%, +17.61%], n=7.

The sign remains positive at 6M, but development uncertainty is wide. The robust claim is therefore bounded to a persistent **equity-over-duration Reflation preference**, strongest at 1M and 3M.

## Promising but weaker relationship — Stagflation: gold over equity
...
However, development confidence intervals cross zero at every horizon. Only the post-2019 6M interval excludes zero. This is economically coherent and worth a constrained secondary test, but it is **not** as well supported as the Reflation equity-vs-duration result.
...
Phase B should use a small number of transparent portfolio templates with a neutral/default allocation for ambiguous regimes. The first test should isolate the best-supported information rather than exploit every observed cell.

A conservative sequence is:

1. neutral diversified portfolio as the default;
2. one Reflation override that overweights equity and underweights long duration;
3. only after measuring that incremental portfolio effect, optionally test a separate Stagflation gold-over-equity defensive override;
4. do not invent separate fitted weights for the remaining seven states.
...
Everything in Phase A is development / reused exploratory historical evidence. The 2020–2026 slice is **not** newly untouched OOS. No production V6.6 parameter changed.
