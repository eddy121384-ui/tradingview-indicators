...
path, rather than subtracting only status-True return rows. Each leaveout path
is then compared with its own realized-exposure-matched control so the reported
residual is timing attribution rather than the exposure change created by
...
def build_leaveout_realized_exposure_matched_control(
    ...
    *,
    ...
    """Match a fresh static control to the realized exposure of one leaveout path.

    Removing an override episode changes realized average invested weights. Reusing
    the original strategy's matched control would therefore mix exposure and timing.
    This helper solves a new monthly-rebalanced static target on the same evaluation
    segment so leaveout residuals remain exposure-neutral post-hoc attribution.
    """
    start_str = None if segment_start is None else segment_start.date().isoformat()
    end_str = None if segment_end is None else segment_end.date().isoformat()
    leaveout_seg = segment_sim(leaveout, start_str, end_str)
    if leaveout_seg.empty:
        raise ValueError("leaveout segment cannot be empty")
    returns_seg = returns.loc[leaveout_seg.index]
    desired = mean_invested_weights(leaveout_seg)
    target_weights, control_seg, metadata = solve_static_target_for_realized_average(
        returns_seg,
        desired,
        cost_bps=cost_bps,
        name=name,
    )
    if not leaveout_seg.index.equals(control_seg.index):
        raise AssertionError("leaveout and leaveout-matched control indices differ")
    metadata = {
        **metadata,
    ...
    return leaveout_seg, control_seg, metadata
...
def run_diagnostics(phase_b_dir: Path) -> dict:
    ...
    for label, (start, end) in segments.items():
        ...
        leaveout_seg, leaveout_control_seg, leaveout_match_meta = (
            build_leaveout_realized_exposure_matched_control(
                ...
                segment_end=end,
                cost_bps=cost_bps,
                name=f"phase_b_leaveout_exposure_match_{label}",
            )
        )
        ...
        leaveout_active_log = float(
            np.log1p(leaveout_seg["net_return"]).sum()
            - np.log1p(leaveout_control_seg["net_return"]).sum()
        )
        ...
        leaveout_metrics = metric_delta(
            ...
            portfolio_metrics(leaveout_control_seg, annualization=252),
        )
        ...
        summary.update(
            {
                ...
                "leaveout_realized_exposure_matching": leaveout_match_meta,
                "leaveout_method": (
                    "disable selected Reflation override episode, rerun full portfolio path, "
                    "then solve a fresh realized-exposure-matched control for that leaveout segment"
                ),
            }
        )
    ...
    result = {
        ...
        "control": (
            "posthoc realized-exposure-matched allocation; each leaveout path is rematched "
            "to its own realized average invested exposure"
        ),
