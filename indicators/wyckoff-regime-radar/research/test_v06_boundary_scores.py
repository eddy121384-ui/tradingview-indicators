...
class V06MechanicalGeneratorTests(unittest.TestCase):
    ...
    def test_generator_replaces_only_the_named_boundary_lines(self) -> None:
    ...
    def test_generated_core_executes_without_touching_v05_module(self) -> None:
        ...
        self.assertIn("no_break_low_score", result.columns)
        self.assertIn("no_break_high_score", result.columns)
        finite_low = result["no_break_low_score"].dropna()
        finite_high = result["no_break_high_score"].dropna()
        self.assertGreater(len(finite_low), 0)
        self.assertGreater(len(finite_high), 0)
        self.assertTrue(((finite_low >= 0.0) & (finite_low <= 100.0)).all())
        self.assertTrue(((finite_high >= 0.0) & (finite_high <= 100.0)).all())
