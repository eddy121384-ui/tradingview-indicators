...
p1h = plot(issue68B312Ready ? yRaw + h : na, "B312 RAW top", color=color.new(colNeutral, 100))
p1l = plot(issue68B312Ready ? yRaw - h : na, "B312 RAW bottom", color=color.new(colNeutral, 100))
p2h = plot(issue68B312Ready ? yBreak + h : na, "B312 BREAK top", color=color.new(colNeutral, 100))
p2l = plot(issue68B312Ready ? yBreak - h : na, "B312 BREAK bottom", color=color.new(colNeutral, 100))
p3h = plot(issue68B312Ready ? yStruct + h : na, "B312 STRUCTURE top", color=color.new(colNeutral, 100))
p3l = plot(issue68B312Ready ? yStruct - h : na, "B312 STRUCTURE bottom", color=color.new(colNeutral, 100))
p4h = plot(issue68B312Ready ? y50 + h : na, "B312 MA50 top", color=color.new(colNeutral, 100))
p4l = plot(issue68B312Ready ? y50 - h : na, "B312 MA50 bottom", color=color.new(colNeutral, 100))
p5h = plot(issue68B312Ready ? y200 + h : na, "B312 MA200 top", color=color.new(colNeutral, 100))
p5l = plot(issue68B312Ready ? y200 - h : na, "B312 MA200 bottom", color=color.new(colNeutral, 100))
p6h = plot(issue68B312Ready ? yHand + h : na, "B312 HANDOFF top", color=color.new(colNeutral, 100))
p6l = plot(issue68B312Ready ? yHand - h : na, "B312 HANDOFF bottom", color=color.new(colNeutral, 100))
...
if barstate.islast
    if showIssue68B312Legend
        table.cell(issue68B312Legend, 0, 0, "LAYER", text_color=color.white, bgcolor=color.new(colNeutral, 15))
        table.cell(issue68B312Legend, 1, 0, "NOW", text_color=color.white, bgcolor=color.new(colNeutral, 15))
        table.cell(issue68B312Legend, 0, 1, "TARGET", text_color=color.white, bgcolor=color.new(colNeutral, 45))
        table.cell(issue68B312Legend, 1, 1, issue68B312Direction, text_color=color.white, bgcolor=color.new(colNeutral, 15))
        table.cell(issue68B312Legend, 0, 2, "RAW", text_color=color.white, bgcolor=color.new(colNeutral, 45))
        table.cell(issue68B312Legend, 1, 2, f_issue68B312Text(issue68B312Raw), text_color=color.white, bgcolor=f_issue68B312Color(issue68B312Raw))
        table.cell(issue68B312Legend, 0, 3, "BREAK", text_color=color.white, bgcolor=color.new(colNeutral, 45))
        table.cell(issue68B312Legend, 1, 3, f_issue68B312Text(issue68B312BreakState), text_color=color.white, bgcolor=f_issue68B312Color(issue68B312BreakState))
        table.cell(issue68B312Legend, 0, 4, "STRUCTURE", text_color=color.white, bgcolor=color.new(colNeutral, 45))
        table.cell(issue68B312Legend, 1, 4, f_issue68B312Text(issue68B312StructureState), text_color=color.white, bgcolor=f_issue68B312Color(issue68B312StructureState))
        table.cell(issue68B312Legend, 0, 5, "MA50", text_color=color.white, bgcolor=color.new(colNeutral, 45))
        table.cell(issue68B312Legend, 1, 5, issue68B312MA50 ? "TARGET SIDE" : "OLD SIDE", text_color=color.white, bgcolor=f_issue68B312Color(issue68B312MA50State))
        table.cell(issue68B312Legend, 0, 6, "MA200", text_color=color.white, bgcolor=color.new(colNeutral, 45))
        table.cell(issue68B312Legend, 1, 6, issue68B312MA200 ? "TARGET SIDE" : "OLD SIDE", text_color=color.white, bgcolor=f_issue68B312Color(issue68B312MA200State))
        table.cell(issue68B312Legend, 0, 7, "HANDOFF", text_color=color.white, bgcolor=color.new(colNeutral, 45))
        table.cell(issue68B312Legend, 1, 7, issue68B312Handoff ? "NOW" : "-", text_color=color.white, bgcolor=issue68B312Handoff ? color.new(color.yellow, 10) : color.new(colNeutral, 15))
    else
...
def generate(source: Path) -> str:
    ...
    for token in ("strategy.", "issue68B34A", "issue68B34B", "issue68B34C", "D1B|"):
        if token in out:
            raise RuntimeError(f"forbidden {token}")
    return out

def main() -> None:
    p = argparse.ArgumentParser()
    p.add_argument("--source", type=Path, default=HERE / SOURCE_RELATIVE)
