...
class Issue68RangeGraceTests(unittest.TestCase):
    def run(self, stages, confirm=3, warmup=0):
    ...
    def test_same_side_trend_resets_grace(self):
        r = self.run([5, 1, 4, 6, 1, 4], confirm=3)
    ...
    def test_synthetic_reciprocal_sequence_is_exact(self):
        ...
        right = MIRROR[left]
        a = self.run(left, confirm=3)
        b = self.run(right, confirm=3)
    ...
    def test_warmup_forces_flat(self):
        r = self.run([2, 2, 5, 5], confirm=3, warmup=2)
