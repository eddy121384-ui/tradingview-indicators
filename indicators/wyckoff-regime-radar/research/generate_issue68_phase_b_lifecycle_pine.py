...
Both builds reuse the runtime-validated Issue #66 D1 C-2 Pine calculation core.
They differ only in the strategy declaration; the body is byte-identical.
"""
...
// Issue #68 frozen Issue #61 human-review-v2 lifecycle.
...
if issue68Ready
    ...
    // Hold through Formal 0 / same-side pauses; exit only on opposite family.
    ...
    // Resolve an already-armed Stage1/4 precursor setup.
    ...
    // NEW lifecycle only from precursor context. Exact 1->2 / 4->5 fresh-break
    // transition is accepted; a generic Stage2/5 fresh break is not a flat chase.
    if issue68Pos == 0 and not issue68ClosedThisBar and issue68ArmedDir == 0
...
// TradingView orders; process_orders_on_close=false is set in the declaration.
if issue68Ready
...
def shared_body(source: Path) -> str:
    ...
    for token in ("PARITY formal_id", "screenshot parity checkpoints", "D1B|"):
        if token in out:
            raise RuntimeError(f"Phase-B strategy leaked parity token: {token}")
    for token in (
        "Issue #66 C-2",
        'volumeMode = "Off"',
        'mtfMode = "Off"',
